...
SINGLETON_IMPL_NO_CREATE(PdRosWrapper)

PdRosWrapper* PdRosWrapper::create_instance(const std::string& __tag1, const std::string& __tag2) {
  if (nullptr != s_inst_) {
    LOG_WARNING << "This method 'create_instance()' is called twice.";
  } else {
    s_inst_ = new PdRosWrapper(Label::make_label(__tag1, "wrapper"),
        Label::make_label(__tag2, "wrapper"));
  }
  return s_inst_;
}

PdRosWrapper::PdRosWrapper(const std::string& _robot_tag, const std::string& _ctrl_tag)
  : MiiRobot(Label::make_label(_robot_tag,  "robot")),
    MiiControl(Label::make_label(_ctrl_tag, "control")),
    jnt_reg_res_(nullptr),
    root_robot_(_robot_tag), root_control_(_ctrl_tag),
    jnt_manager_(nullptr), alive_(true), rt_duration_(1000/1000) {

  __setup_env();
}

PdRosWrapper::~PdRosWrapper() {
  alive_ = false;
...
void PdRosWrapper::create_system_singleton() {
...
//void PdRosWrapper::auto_inst() {
//  MiiApp::auto_inst();
//}

bool PdRosWrapper::init() {
  bool verbose = false;
  ...
  ctrl_sub_ = nh_.subscribe<std_msgs::String>(str, 1, &PdRosWrapper::cbForControl, this);
  // For debug
  #ifdef DEBUG_TOPIC
    cmd_sub_ = nh_.subscribe<std_msgs::Float32>("debug", 100,
        &PdRosWrapper::cbForDebug, this);
  #endif
  ...
  cfg->get_value(root_robot_, "frequency", frequency);
  tick_interval_ = std::chrono::microseconds((int)(1000000/frequency));

  // registry the thread.
  ThreadPool::instance()->add("support-registry2", &PdRosWrapper::supportRegistry, this);
  ThreadPool::instance()->add("rt_puber", &PdRosWrapper::publishRTMsg, this);
  return true;
}

void PdRosWrapper::__reg_resource_and_command() {
  auto reg    = Registry2::instance();
  auto cfg    = CfgReader::instance();
  int count   = 0;
  LegType leg = LegType::UNKNOWN_LEG;

  std::string _ptag = Label::make_label(root_robot_, "registry2");
  std::string _ltag = Label::make_label(_ptag, "legs");
  std::string str;
  cfg->get_value(_ltag, "mode", str);

  jnt_reg_res_ = new __RegRes;
...
//  REG_COMMAND_NO_FLAG(str, (int*)(&jnt_reg_res_->cmd_mode));
  ...
  while (cfg->get_value(_legs_tag, "leg", leg)) {
    ...
    if (cfg->get_value(_legs_tag, "vel", str)) {
...
void PdRosWrapper::supportRegistry() {
...
void PdRosWrapper::publishRTMsg() {
...
//    cfg->get_value(Label::make_label(root_tag_, "Pdroswrapper"), "cmds", cmds);
//    FOR_EACH_LEG(l) {
...
void PdRosWrapper::cbForControl(const std_msgs::String::ConstPtr& msg) {
...
#ifdef DEBUG_TOPIC
void PdRosWrapper::cbForDebug(const std_msgs::Float32ConstPtr& msg) {
